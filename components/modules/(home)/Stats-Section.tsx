    ...
    <section id="stats" className="bg-navyMid border-b border-gold400/10 py-10">
      <div className="max-w-7xl mx-auto px-8">
        <div className="grid grid-cols-2 md:grid-cols-4">
          {stats.map((item) => (
            <div key={item.id} className="text-center px-4 py-14 border-r border-white/5 last:border-r-0 ">
              <CountUp
                start={0}
                end={item.value}
                suffix={item.suffix}
              ...
              />
              <h4 className="text-white/50 text-sm mt-2 tracking-[0.05em]">{item.label}</h4>
            </div>
          ))}
        </div>
      </div>
